...
fn scores_that_have_not_moved_do_not_emit() {
    ...
    let quiet = &[][..];
    assert!(
        gate.should_emit(101.4, 98.2, quiet),
        "the first view must be sent"
    );
    assert!(!gate.should_emit(101.4, 98.2, quiet));
    // Below a hundredth of a point is not a score change.
    assert!(!gate.should_emit(101.4001, 98.2001, quiet));
    assert!(gate.should_emit(101.5, 98.2, quiet));
    assert!(
        gate.should_emit(101.5, 98.3, quiet),
...
fn the_scoreboard_moving_at_nil_nil_still_emits() {
    ...
    assert!(gate.should_emit(0.0, 0.0, std::slice::from_ref(&game)));
    assert!(!gate.should_emit(0.0, 0.0, std::slice::from_ref(&game)));
    ...
    assert!(
        gate.should_emit(0.0, 0.0, std::slice::from_ref(&game)),
