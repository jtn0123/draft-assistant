...
Each message carries the current draft state as JSON, then the full available-player board as a \
...
fn board_row(p: &AvailablePlayer) -> String {
    ...
    format!(
        ...
        p.survival_next
            ...
            .unwrap_or_else(|| "-".into()),
        b.injury_status.as_deref().unwrap_or(""),
    )
}
...
pub fn build_prompt(
    ...
    let state = state_json(view)?;
    let board = board_table(&view.available);
    let mut prompt = format!("Current draft state:\n```json\n{state}\n```\n\n{board}\n");
