...
| 5 | Compare origin lineup advice vs local `season_lineup.rs` | analysis done | Local wins decisively (origin has the slot-index pairing bug local's tests kill). Two grafts in flight with item 6: per-position variance + stack correlation, and zeroing sidelined players before the optimal solve. |
| 6 | Review t3code branch for porting | analysis done, ports in flight | Port-worthy: calibrated spread constants (fitted out-of-sample on 1,746 player-weeks), `pick_value.rs`, backtest harness (follow-up), season-aware sync-badge threshold (check local). Skipped: its wdio e2e (local's is better organised), trade-offer UI (needs origin's whole evaluator). |
