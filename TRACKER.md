...
| 5 | D1: second-opinion projections column on the draft board (Clay rank next to the app's, flag ≥8-spot disagreements, rec-card reason) | in progress | agent `second-opinion` |
| 6 | Rust coverage to 80% with a threshold enforced the way `vitest.config.ts` enforces the frontend's | in progress | agent `rust-coverage` |
